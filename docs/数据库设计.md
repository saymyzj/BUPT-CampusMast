...
| latitude | decimal(10,7) | 楼宇中心纬度 |
| longitude | decimal(10,7) | 楼宇中心经度 |
| polygon_json | json nullable | 楼宇轮廓多边形，经纬度数组，首版允许为空 |
| is_active | boolean | 是否启用 |
| created_at | datetime | 创建时间 |
| updated_at | datetime | 更新时间 |
...
| latitude | decimal(10,7) | 用户地图选点纬度；为空时使用楼宇中心点 |
| longitude | decimal(10,7) | 用户地图选点经度；为空时使用楼宇中心点 |
...
| provider | varchar(50) | `LOCAL_KEYWORD` / `DEEPSEEK` / `FALLBACK` |
| risk_level | enum | `ALLOW` / `REVIEW` / `BLOCK` |
| hit_tags | json | 命中标签；refuse/reject 可包含 `risk:LOW` / `risk:MEDIUM` / `risk:HIGH` |
| model_output | json | 审核输出摘要，包含 `aiResult`、`riskHint`、`reason` |
| admin_review_status | enum | `PENDING` / `APPROVED` / `REJECTED`；ALLOW 自动 APPROVED，BLOCK 自动 REJECTED，REVIEW 待人工复审 |
